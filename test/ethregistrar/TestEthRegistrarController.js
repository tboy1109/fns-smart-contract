...
const ETHRegistrarController = artifacts.require('./FTMRegistrarController');
...
contract('FTMRegistrarController', function (accounts) {
    let ens;
    let resolver;
    let baseRegistrar;
    let controller;
    ...
    before(async () => {
        ...
        baseRegistrar = await BaseRegistrar.new(ens.address, namehash.hash('ftm'), {from: ownerAccount});
        await ens.setSubnodeOwner('0x0', sha3('ftm'), baseRegistrar.address);
    ...
    it('should report registered names as unavailable', async () => {
        assert.equal(await controller.available('newname'), true);
    });
    ...
    it('should permit new registrations with config', async () => {
        ...
        // var nodehash = namehash.hash("newconfigname.ftm");
    ...
    it('should permit a registration with resolver but not addr', async () => {
        ...
        // var nodehash = namehash.hash("newconfigname2.ftm");
        // assert.equal((await ens.resolver(nodehash)), resolver.address);
