...
describe('Name Wrapper', () => {
  ...
  before(async () => {
    ...
    EnsRegistry = await deploy('FNSRegistry')
    ...
    await EnsRegistry.setSubnodeOwner(
      ROOT_NODE,
      utils.keccak256(utils.toUtf8Bytes('ftm')),
      BaseRegistrar.address
    )
    ...
    expect(await EnsRegistry.owner(namehash('ftm'))).to.equal(
      BaseRegistrar.address
    )
  })
  ...
  describe('wrap()', () => {
    ...
    it('Does not allow wrapping .eth 2LDs.', async () => {
      ...
      await EnsRegistry.setApprovalForAll(NameWrapper.address, true)
      // await expect(
        // NameWrapper.wrap(encodeName('wrapped.ftm'), account2, 0, EMPTY_ADDRESS)
      // ).to.be.revertedWith(
  ...
  describe('unwrap()', () => {
    ...
    it('Will not unwrap .ftm 2LDs.', async () => {
      ...
      const ownerOfWrappedETH = await NameWrapper.ownerOf(
        namehash('unwrapped.ftm')
      )
      expect(ownerOfWrappedETH).to.equal(account)
      await expect(
        NameWrapper.unwrap(namehash('ftm'), labelhash('unwrapped'), account)
      ).to.be.revertedWith(
  ...
  describe('wrapETH2LD()', () => {
    ...
    const labelHash = labelhash(label)
    const nameHash = namehash(label + '.ftm')
    ...
    it('Can re-wrap a name that was wrapped has already expired', async () => {
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('wrapped2.ftm'), EMPTY_ADDRESS)
      await expect(tx)
        .to.emit(NameWrapper, 'TransferSingle')
        .withArgs(account2, account, EMPTY_ADDRESS, nameHash, 1)
      await expect(tx)
        .to.emit(NameWrapper, 'NameWrapped')
        .withArgs(
          namehash('wrapped2.ftm'),
          encodeName('wrapped2.ftm'),
          account2,
          CAN_DO_EVERYTHING
        )
      await expect(tx)
    ...
    it('correctly reports fuses for a name that has expired and been rewrapped more permissively', async () => {
      ...
      expect(fuses).to.equal(CANNOT_UNWRAP)
      expect(vulnerability).to.equal(ParentVulnerability.Fuses)
      expect(nodeVulnerable).to.equal(namehash('wrapped2.ftm'))
    })
  ...
  describe('unwrapETH2LD()', () => {
    ...
    it('Allows the owner to unwrap a name.', async () => {
      ...
      expect(await NameWrapper.ownerOf(namehash('unwrapped.ftm'))).to.equal(
        account
      )
      ...
      expect(await EnsRegistry.owner(namehash('unwrapped.ftm'))).to.equal(
        account
      )
    ...
    it('emits Unwrap event', async () => {
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('unwrapped.ftm'), account)
    })
  ...
  describe('ownerOf()', () => {
    it('Returns the owner', async () => {
      ...
      const tokenId = labelhash(label)
      const wrappedTokenId = namehash(label + '.ftm')
      const CAN_DO_EVERYTHING = 0
  ...
  describe('burnFuses()', () => {
    ...
    const wrappedTokenId = namehash('fuses.ftm')
    ...
    it('can set fuses and burn transfer', async () => {
      ...
      const wrappedTokenId = namehash('fuses3.ftm')
    ...
    it('can set fuses and burn canCreateSubdomains', async () => {
      ...
      expect(
        await EnsRegistry.owner(namehash('creatable.fuses2.ftm'))
      ).to.equal(NameWrapper.address)

      expect(
        await NameWrapper.ownerOf(namehash('creatable.fuses2.ftm'))
      ).to.equal(account)
  ...
  describe('setSubnodeOwnerAndWrap()', async () => {
    ...
    it('Can be called by the owner of a name and sets this contract as owner on the ENS registry.', async () => {
      ...
      expect(await EnsRegistry.owner(namehash(`sub.${label}.ftm`))).to.equal(
        NameWrapper.address
      )

      expect(await NameWrapper.ownerOf(namehash(`sub.${label}.ftm`))).to.equal(
        account
      )
    })
    it('Can be called by an account authorised by the owner.', async () => {
  ...
  describe('setSubnodeRecordAndWrap()', async () => {
    ...
    it('Sets the appropriate values on the ENS registry', async () => {
      ...
      const node = namehash(`sub.${label}.ftm`)
  ...
  describe('setSubnodeRecord', () => {
    ...
    const subWrappedTokenId = namehash(`${subLabel}.${label}.ftm`)
  ...
  describe('onERC721Received', () => {
    ...
    it('Reverts if called by anything other than the FNS registrar address', async () => {
      await BaseRegistrar.register(tokenId, account, 84600)
  ...
  describe('getFuses', () => {
    ...
    const subLabel = 'sub'
    const subLabelHash = labelhash(subLabel)
    const subNameHash = namehash(`${subLabel}.${label}.ftm`)
    const subSubLabel = 'subsub'
    const subSubLabelhash = labelhash(subSubLabel)
    const subSubNameHash = namehash(`${subSubLabel}.${subLabel}.${label}.ftm`)
