...
contract HashRegistrar is Registrar {
    ...
    modifier registryOpen() {
        require(block.timestamp >= registryStarted && block.timestamp <= registryStarted + (365 * 4) * 1 days && fns.owner(rootNode) == address(this));
        _;
    }
     ...
     *
     * @param _fns The address of the FNS
     * @param _rootNode The hash of the rootnode.
     */
    constructor(FNS _fns, bytes32 _rootNode, uint _startDate) public {
        fns = _fns;
        rootNode = _rootNode;
     ...
     */
    function startAuction(bytes32 _hash) external virtual override {
        _startAuction(_hash);
    }
     ...
     */
    function startAuctions(bytes32[] calldata _hashes) external virtual override {
     ...
     * Since this is an auction, it is expected that most public hashes, like kblock.timestampn domains and common dictionary
     * words, will have multiple bidders pushing the price up.
     *
     * @param sealedBid A sealedBid, created by the shaBid function
     */
    function newBid(bytes32 sealedBid) external virtual override payable {
        _newBid(sealedBid);
    }
     ...
     */
    function startAuctionsAndBid(bytes32[] calldata hashes, bytes32 sealedBid) external virtual override payable {
     ...
     */
    function unsealBid(bytes32 _hash, uint _value, bytes32 _salt) external virtual override {
     ...
     */
    function cancelBid(address bidder, bytes32 seal) external virtual override {
        ...
        bid.closeDeed(5);
        sealedBids[bidder][seal] = Deed(address(0x0));
     ...
     */
    function finalizeAuction(bytes32 _hash) external virtual override onlyOwner(_hash) {
        Entry storage h = _entries[_hash];
     ...
     */
    function transfer(bytes32 _hash, address payable newOwner) external virtual override onlyOwner(_hash) {
     ...
     */
    function releaseDeed(bytes32 _hash) external virtual override onlyOwner(_hash) {
        Entry storage h = _entries[_hash];
        Deed deedContract = h.deed;

        require(block.timestamp >= h.registrationDate + 365 days || fns.owner(rootNode) != address(this));
     ...
     */
    function eraseNode(bytes32[] calldata labels) external virtual override {
     ...
     */
    function transferRegistrars(bytes32 _hash) external virtual override onlyOwner(_hash) {
        address registrar = fns.owner(rootNode);
     ...
     */
    function acceptRegistrarTransfer(bytes32 hash, Deed deed, uint registrationDate) external virtual override {
        hash; deed; registrationDate; // Don't warn about unused variables
    }

    function entries(bytes32 _hash) external virtual override view returns (Mode, address, uint, uint, uint) {
        Entry storage h = _entries[_hash];
    ...
    function state(bytes32 _hash) public virtual override view returns (Mode) {
    ...
    function _tryEraseSingleNode(bytes32 label) internal {
        if (fns.owner(rootNode) == address(this)) {
            fns.setSubnodeOwner(rootNode, label, address(this));
            bytes32 node = keccak256(abi.encodePacked(rootNode, label));
            fns.setResolver(node, address(0x0));
            fns.setOwner(node, address(0x0));
        }
    }
    ...
    function _eraseNodeHierarchy(uint idx, bytes32[] memory labels, bytes32 node) internal {
        ...
        fns.setSubnodeOwner(node, labels[idx], address(this));
    ...
    /**
     * @dev Assign the owner in FNS, if we're still the registrar
     *
    ...
    function trySetSubnodeOwner(bytes32 _hash, address _newOwner) internal {
        if (fns.owner(rootNode) == address(this))
            fns.setSubnodeOwner(rootNode, _hash, _newOwner);
    }
