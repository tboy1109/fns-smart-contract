    ...
    IERC721Receiver
{
    ...
    FNS public immutable override fns;
    BaseRegistrar public immutable override registrar;
    IMetadataService public override metadataService;
    mapping(bytes32 => bytes) public override names;

    bytes32 private constant FTM_NODE =
        0xef0d8150af51886892faa1bf5f9cb425c4c2194294fbea6519ce2a9cff9fb4d8;
    ...
    constructor(
        ...
        /* Burn CANNOT_REPLACE_SUBDOMAIN and CANNOT_UNWRAP fuses for ROOT_NODE and FTM_NODE */
    ...
    /**
     * @notice Wraps a .ftm domain, creating a new token and sending the original ERC721 token to this *         contract
     * @dev Can be called by the owner of the name in the .ftm registrar or an authorised caller on the *      registrar
     * @param label label as a string of the .ftm domain to wrap
    ...
    function wrapETH2LD(
        ...
        require(
            ...
            "NameWrapper: Sender is not owner or authorised by the owner or authorised on the .ftm registrar"
        );
    ...
    /**
     * @dev Registers a new .ftm second-level domain and wraps it.
     *      Only callable by authorised controllers.
     * @param label The label to register (Eg, 'foo' for 'foo.ftm').
    ...
    /**
     * @dev Renews a .ftm second-level domain.
     *      Only callable by authorised controllers.
     * @param tokenId The hash of the label to register (eg, `keccak256('foo')`, for 'foo.ftm').
    ...
    /**
     * @notice Wraps a non .ftm domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    ...
    function wrap(
        ...
        require(
            parentNode != FTM_NODE,
            "NameWrapper: .ftm domains need to use wrapETH2LD()"
        );
    ...
    /**
     * @notice Unwraps a .ftm domain. e.g. vitalik.ftm
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
     * @param label label as a string of the .ftm domain to wrap e.g. vitalik.xyz would be 'vitalik'
     * @param newRegistrant sets the owner in the .ftm registrar to this address
    ...
    ) public override onlyTokenOwner(_makeNode(FTM_NODE, label)) {
        _unwrap(_makeNode(FTM_NODE, label), newController);
        registrar.transferFrom(address(this), newRegistrant, uint256(label));
    }

    /**
     * @notice Unwraps a non .ftm domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
     * @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    ...
    function unwrap(
        ...
        require(
            parentNode != FTM_NODE,
            "NameWrapper: .ftm names must be unwrapped with unwrapETH2LD()"
        );
    ...
    /**
     * @notice Sets records for the subdomain in the FNS Registry
    ...
    {
        fns.setSubnodeRecord(parentNode, label, owner, resolver, ttl);
    }
    ...
    function setSubnodeOwner(
        ...
        returns (bytes32)
    {
        return fns.setSubnodeOwner(parentNode, label, owner);
    }
    ...
    {
        fns.setRecord(node, owner, resolver, ttl);
    }
    ...
    modifier canCallSetSubnodeOwner(bytes32 node, bytes32 label) {
        ...
        address owner = fns.owner(subnode);
        (, uint96 fuses) = getData(uint256(node));
    ...
    function onERC721Received(
        ...
        //check if it's the ftm registrar ERC721
        require(
            msg.sender == address(registrar),
            "NameWrapper: Wrapper only supports .ftm ERC721 token transfers"
        );
    ...
    function _wrapETH2LD(
        ...
        bytes32 node = _makeNode(FTM_NODE, labelhash);
        bytes memory name = _addLabel(label, "\x03ftm\x00");
    ...
    function _checkOwnership(
        ...
        if (parentNode == FTM_NODE) {
            // Special case .ftm: Check registrant or name isexpired
        ...
        if (fns.owner(node) != address(this)) {
